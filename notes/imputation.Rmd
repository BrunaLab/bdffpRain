...
- Add south america gridded data (daily)
...
```{r}
cor(full_wide$manaus, full_wide$km_clust, method = "spearman", use = "pairwise.complete.obs")
cor(full_wide$manaus, lag(full_wide$km_clust), method = "spearman", use = "pairwise.complete.obs")
```

Nope, seems like it's fine the way it is.  There is a much weaker correlation if I offset by a day.
...
slice_gaps <- 
  slice_gaps %>% 
  mutate(manaus_back = spread_back(manaus, max_n = 15),
...
pmat <-
  ...
  add_column(
    # sd = sd(slice_gaps$manaus, na.rm = TRUE),
    sd = 1,
    colid = 8
    ) %>% 
...
Because the spread-out values are low, they only pull the predictions toward zero.  This is probably not going to be helpful.
