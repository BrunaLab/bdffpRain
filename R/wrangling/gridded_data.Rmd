...
I did this before with a command line utility, but someone suggested `raster::stack`
...
plot(r.t.flipxy)
```
Try as a raster stack

```{r}
rs <- raster::stack(outpaths[1:5])
rs2 <- rs %>% t() %>% flip(2) %>% flip(1)
crs(rs2) <- 4326

names(rs2) <- ymd(dates)[1:5]
rs2
```

```{r}
test <- getValues(rs2$X1998.01.01, format = "matrix")
colnames(test) <- xFromCol(rs2$X1998.01.01)
rownames(test) <- yFromRow(rs2$X1998.01.01)
test
precip_list <-
  map(outpaths, ~{
    r <- raster(.x) %>% 
...
getCHIRPS(AOI, startDate = "1999-01-01", endDate = "2001-01-01")
