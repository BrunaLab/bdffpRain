...
cdo mergetime 1980_1989.nc 1990_1999.nc 2000_2009.nc 2010_2015.nc all_precip.nc
rm 1980_1989.nc 1990_1999.nc 2000_2009.nc 2000_2009.nc 2010_2015.nc
```

### TODO:

- Repeat this with other variables (RH, ET, Temp), then use `cdo merge` to merge files with different variables.

## Tidy

```{r}
xa_precip <- nc_open(here("data_raw", "XavierUT", "all_precip.nc"))
prec <- ncvar_get(xa_precip, "prec")
dim(prec)
time <- ncvar_get(xa_precip, "time") #hours since 1980-01-01 12:00:00
time <- as_date(time/24, origin = ymd("1980-01-01"))

dimnames(prec) <- list(ncvar_get(xa_precip, "longitude"),
                       as.character(time))
xa_precip_out <-
  ...
  add_column(lat = ncvar_get(xa_precip, "latitude")) %>% 
  mutate(date = ymd(date), lon = as.numeric(lon)) %>% 
  select(date, lat, lon, precip)
```

```{r}
write_csv(xa_precip_out, here("data_cleaned", "xavier_daily_precip_0.25x0.25.csv"))
```
