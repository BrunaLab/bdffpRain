...
bdffp_ts %>% 
  ...
  index_by(month = month(yearmonth, label = TRUE)) %>% 
  group_by(site) %>% 
  summarize(mean_precip = mean(precip)) %>% 
  ggplot(aes(x = month, y = mean_precip, color = site)) + 
    geom_point()
```
```{r}
bdffp_ts2 %>% 
...
# anyNA(bdffp_mon$tot_precip)
# any(is.nan(bdffp_mon$tot_precip))
```
...
NAs in gaps less than 3 days are ignored.  Longer runs of NAs or NAs that cross month boundaries invalidate monthly total.  To replicate this in R I need to do something so small groups of NAs are just removed, and values after large groups of NAs or ones that cross month borders are set to NA.  Keeping the NAs in these gaps will invalidate the monthly totals if na.rm = FALSE.
...
bdffp_ts <- 
  bdffp_ts %>% 
  group_by(site) %>% 
  mutate(gap_len2 = ifelse(!is.na(precip) & is.na(gap_len), 0, gap_len)) %>% 
  fill(gap_len2, .direction = "up")
```


### Set unallowed accumulations to NA
#### gaps larger than 2 days

```{r}
bdffp_noaa <- 
  bdffp_ts %>%
  mutate(precip = ifelse(gap_len > allowed_gap & !is.na(gap_len), NA, precip))
```

#### gaps that cross months
The way this works is if the difference between the month on the day of the accumulated observation is different from the month at the beginning of the NAs, then that observation is removed

This should only apply to gaps longer than a day, I think.
...
bdffp_noaa <-
  ...
  mutate(
    x =
      ifelse(
        !is.na(gap_len) & gap_len > 1,
        cur_data()$month[rowid] - cur_data()$month[rowid-gap_len],
        0
      )
    ) %>% 
  mutate(precip = ifelse(x != 0,
                         NA,
                         precip)) %>% 
  select(-month, -x)
```

### Remove NAs for small gaps
Remove NAs for gaps shorter than 2 days (might want to go to 3 or 4 later)

```{r}
bdffp_noaa <-
  bdffp_noaa %>% 
  mutate(gap_len2 = ifelse(!is.na(gap_len), 0, gap_len2)) %>% 
  filter(gap_len2 == 0 | gap_len2 > allowed_gap)
unique(bdffp_noaa$site)
```
...
noaa_tot %>% 
  as_tibble() %>% 
  group_by(site) %>% 
  summarize(complete = sum(!is.na(tot_precip))/n()*100)
```
yeeesh, using NOAA rules of thumb, km41 is 28% complete, but some sites have zero months. Even increasing to 14 days allowable gap doesn't change anything.
