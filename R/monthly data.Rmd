...
Another possibility is to spread accumulated values for gaps smaller than 3 days (like NOAA), and remove accumulations for larger gaps.  Then, impute all missing values in some way.  I could use the Manaus and/or GPCC data to test different imputation methods by removing values, imputing them, and calculating RMSE.  The [`imputeTestbench`](https://cran.r-project.org/web/packages/imputeTestbench/index.html) package might help with this, although it sounds like it only works with univariate imputation.

Regardless, it seems like it would be helpful to have some test set that I could then use to check the effectiveness of various methods.
...
```{r}
class(manaus)
manaus.ts <- as.ts(manaus_ts %>% select(date, precip))
aa <- impute_errors(manaus.ts, smps = "mar", blck = 7, blckper = FALSE) #i think this is close to median gap length
plot_errors(aa)
```

I think I should consider multivariate imputation treating each site as a variable.  The `mice` package can do this, but doesn't know about time series.  It sounds like the [`mtsdi`](https://cran.r-project.org/web/packages/mtsdi/index.html) package is built for imputation of multivariate time series and draws on information from other sites *and* previous time periods to impute missing values.  There is another package, [`amelia`](https://gking.harvard.edu/amelia) that appears to do something similar.
