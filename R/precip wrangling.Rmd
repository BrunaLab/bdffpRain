...
#typo in Dimona
all_clean2 <- all_clean2 %>% mutate(precip = replace(precip, rowname == 27449, 53))
```
## Problems with dates
...
typos <- c("12573" = ymd("1988-10-01"),
           "32251" = ymd("1992-10-14"),
           "39061" = ymd("1994-01-05"),
           "39063" = ymd("1994-01-07"),
           "38619" = ymd("1992-10-20"))
...
all_clean2 <- all_clean2 %>% filter(rowname != 10532)
```
...
all_clean2 <- 
  ...
  mutate(flag = case_when(
    str_detect(notes, "(A|a|Á)c{1,2}umu") ~ "accumulated",
...
all_clean2 <-
  all_clean2 %>% 
  mutate(precip = ifelse(str_detect(notes, "nao foi feita observ") & precip == 0, NA, precip)) 
```
